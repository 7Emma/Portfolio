...
const LoadingSpinner = ({ fullScreen = true }) => {
  // Classe pour le conteneur - toujours adaptée à tous les écrans
  const containerClass = fullScreen 
    ? "fixed inset-0 flex justify-center items-center w-full h-full bg-black z-50" 
    : "flex justify-center items-center w-full p-4 bg-black";

  return (
    <div className={containerClass}>
      <div className="relative">
        {/* Effet de lueur externe - taille responsive */}
        <div className="absolute inset-0 blur-md bg-green-500 bg-opacity-20 rounded-full"></div>
        
        {/* Spinner avec effet néon - complètement responsive */}
        <svg
          aria-hidden="true"
          className="w-10 h-10 xs:w-12 xs:h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 xl:w-28 xl:h-28 
                    animate-spin text-green-400 drop-shadow-[0_0_10px_#00FF00] fill-green-500"
          viewBox="0 0 100 101"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          {/* Cercle extérieur - effet néon subtil */}
          <path
          ...
          />
          {/* Partie animée - vert néon brillant */}
          <path
        ...
        {/* Texte "Chargement..." en vert néon - responsive */}
        <span className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs sm:text-sm md:text-base text-green-400 font-medium drop-shadow-[0_0_2px_#00FF00]">
          Chargement...
        </span>
      </div>
    </div>
